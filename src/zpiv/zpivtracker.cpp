// Copyright (c) 2018-2019 The Puppycoin developers
...
CzPUPTracker::CzPUPTracker(std::string strWalletFile)
{
...
CzPUPTracker::~CzPUPTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzPUPTracker::Init()
{
...
bool CzPUPTracker::Archive(CMintMeta& meta)
{
...
bool CzPUPTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzPUPTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzPUPTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzPUPTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzPUPTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzPUPTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzPUPTracker::GetSerialHashes()
{
...
CAmount CzPUPTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzPUPTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzPUPTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzPUPTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzPUPTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzPUPTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzPUPTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzPUPTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzPUPTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzPUPTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzPUPTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzPUPWallet* zPUPWallet)
{
    bool iszPUPWalletInitialized = (NULL != zPUPWallet);
    CMintMeta meta;
    ...
    if (! iszPUPWalletInitialized)
        zPUPWallet = new CzPUPWallet(strWalletFile);
    meta.isSeedCorrect = zPUPWallet->CheckSeed(dMint);
    if (! iszPUPWalletInitialized)
        delete zPUPWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzPUPTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzPUPTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzPUPTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzPUPTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzPUPTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzPUPTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzPUPWallet* zPUPWallet = new CzPUPWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zPUPWallet);
        }
        delete zPUPWallet;
