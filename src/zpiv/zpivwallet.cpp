...
CzPUPWallet::CzPUPWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPUPSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPUPSeed_deprecated()) {
                    LogPrintf("%s: Updated zPUP seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzPUPWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPUPCount(nCountLastUsed);
    else if (!walletdb.ReadZPUPCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPUPWallet::Lock()
{
    seedMaster = 0;
}

void CzPUPWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPUPWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPUP(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPUPWallet::LoadMintPoolFromDB()
{
...
void CzPUPWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPUPWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPUPWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzPUPWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPUP(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzPUPWallet::SeedToZPUP(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzPUPWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPUPWallet::GenerateDeterministicZPUP(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPUPWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPUPWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzPUPWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
