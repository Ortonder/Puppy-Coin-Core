...
public:
    ...
    bool WriteZPUPSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZPUPSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZPUPSeed_deprecated(uint256& seed);
    bool EraseZPUPSeed();
    bool EraseZPUPSeed_deprecated();

    bool WriteZPUPCount(const uint32_t& nCount);
    bool ReadZPUPCount(uint32_t& nCount);
