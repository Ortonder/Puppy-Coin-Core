    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zPUP.";
            else if (!mint.isSeedCorrect)
                strReason = "The zPUP seed used to mint this zPUP is not the same as currently hold in the wallet";
            else
